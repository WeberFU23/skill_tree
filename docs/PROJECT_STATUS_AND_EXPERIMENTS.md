...
| `scripts/eval_locomo_skilltree_negmem_curated_agg055_cat3match.sh` | Test curated aggregate retrieval with category matching only for category 3 |
...
16. The selective category-2/3 repeat averaged F1 0.2105 +/- 0.0185 and LLM
    Judge 0.2983 +/- 0.0314. This is useful for analysis and improves over
    all-category matching, but it does not clearly beat ordinary curated
    retrieval on F1.
17. The category-wise repeat summary for category-2/3 matching is Cat1
...
19. Question-level negative-memory impact analysis identified three
    ...
    three lessons.
20. Pruning those three lessons produced the strongest negative-memory-only
    curated candidate:
    F1 0.2309 +/- 0.0169 and LLM Judge 0.3095 +/- 0.0106. Category-wise,
...
23. Repeating the pruned-bad3 evolved checkpoint produced the strongest repeated
    setting so far: F1 0.2377 +/- 0.0103 and LLM Judge 0.3217 +/- 0.0145.
...
8. Test the pruned curated directory built from the question-level diagnostic:
   ...
   ```
9. Compare pruned bad3 against raw top2 at the question level, especially for
   category 2, before further pruning or promotion:
...
12. Summarize and compare the evolved-checkpoint repeat before deciding the
   next training change:
